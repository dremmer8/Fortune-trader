...
// Save game state - Singleplayer mode: localStorage only
...
async function saveGameState() {
    ...
    // Save to localStorage (singleplayer mode - no cloud sync)
    try {
...
// Check and sync pending save snapshot from previous session (singleplayer - no sync needed)
async function syncPendingSaveSnapshot() {
    // Singleplayer mode - no cloud sync, just clear any pending snapshots
    localStorage.removeItem(PENDING_SAVE_KEY);
    return { hadPendingSave: false };
}

// Load game state - Singleplayer mode: localStorage only
async function loadGameState() {
    ...
    // Restore state if we found data
    if (saveData) {
...
// Reset game (singleplayer - no cloud data to delete)
